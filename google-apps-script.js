  ...
  try {
    ...
    switch (action) {
      ...
      case 'testConnection':
        ...
        break;
      default:
        result = { success: false, error: 'Invalid action for GET request: ' + action };
    }
...
function getPlayers() {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      if (row && row[0]) { // Check if ID exists
...
function savePlayer(player) {
  try {
    ...
    if (!sheet) {
      ...
      sheet.getRange(1, 1, 1, 6).setValues([['ID', 'Name', 'Position', 'SkillPoints', 'Image', 'CreatedAt']]);
    }
    ...
    sheet.appendRow([
      ...
      clampedSkillPoints,
      String(player.image || ''),
      parseInt(player.createdAt) || Date.now()
    ]);
  ...
  try {
    ...
    if (!found) {
      return { 
        ...
        searchedIds: data.slice(1).map(row => String(row[0] || '')).slice(0, 5) // First 5 IDs for debugging
      };
    }
...
function saveMatch(match) {
  try {
    ...
    if (!sheet) {
      ...
      sheet.getRange(1, 1, 1, 9).setValues([['ID', 'Team1', 'Team2', 'Score1', 'Score2', 'Date', 'Team1Players', 'Team2Players', 'CreatedAt']]);
    }
...
function getTeams() {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      if (row && row[0]) { // Check if TeamName exists
        try {
          teams.push({
            name: String(row[0] || ''),
...
function saveTeams(teams) {
  try {
    ...
    if (!sheet) {
      ...
      sheet.getRange(1, 1, 1, 4).setValues([['TeamName', 'Players', 'TotalPoints', 'CreatedAt']]);
    }
...
function setupSheets() {
  try {
    ...
    if (!playersSheet) {
      ...
      playersSheet.getRange(1, 1, 1, 6).setValues([['ID', 'Name', 'Position', 'SkillPoints', 'Image', 'CreatedAt']]);
    }
    ...
    if (!matchesSheet) {
      ...
      matchesSheet.getRange(1, 1, 1, 9).setValues([['ID', 'Team1', 'Team2', 'Score1', 'Score2', 'Date', 'Team1Players', 'Team2Players', 'CreatedAt']]);
    }
    ...
    if (!teamsSheet) {
      ...
      teamsSheet.getRange(1, 1, 1, 4).setValues([['TeamName', 'Players', 'TotalPoints', 'CreatedAt']]);
    }
